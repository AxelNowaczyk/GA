...
//  MinOfFunctionGA
//
//  Created by Axel Nowaczyk on 13.03.2016.
...
class AnalizeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        var avgFits = [Double]()
        var sum: Double = 0
        var best: Double = simpleFunction.population[0].fitness!
        var worst: Double = simpleFunction.population[0].fitness!
        for index in 0..<maxGeneration{
            ...
            avgFits.append(sum/Double(simpleFunction.population.count))
            simpleFunction.doGeneration()
            best = simpleFunction.population[0].fitness!
            worst = simpleFunction.population[0].fitness!
            sum = 0
        }
